 ...
 */
final class program extends item {
    /** @var string item type */
    public const TYPE = 'program';
    ...
    protected static function pre_update(stdClass $record, stdClass $section, stdClass $data, stdClass $oldrecord): void {
        ...
        $program = $DB->get_record('tool_muprog_program', ['id' => $oldrecord->referenceid]);
        if (!$program) {
            return;
        }
    ...
    public static function is_restore_possible(stdClass $item): bool {
        ...
        $sql = "SELECT 'x'
                  ...
                  JOIN {tool_muprog_program} p ON p.id = i.referenceid
                 WHERE i.id = :id AND i.type = 'program'";
        return $DB->record_exists_sql($sql, ['id' => $item->id]);
    }

    /**
     * Program updated observer - syncs item names.
     *
     * @param \tool_muprog\event\program_updated $event
     * @return void
     */
    public static function event_program_updated(\tool_muprog\event\program_updated $event): void {
        global $DB;

        $program = $event->get_record_snapshot('tool_muprog_program', $event->objectid);

        $sql = "SELECT i.id
                  FROM {tool_mucatalog_item} i
                 WHERE i.type = 'program' AND i.referenceid = :programid AND i.syncname = 1 AND i.name <> :name
              ORDER BY i.id ASC";
        $ids = $DB->get_fieldset_sql($sql, ['programid' => $program->id, 'name' => $program->fullname]);
        foreach ($ids as $id) {
    ...
    /**
     * program deleted observer - archives/deletes items.
     *
     * @param \tool_muprog\event\program_deleted $event
     * @return void
     */
    public static function event_program_deleted(\tool_muprog\event\program_deleted $event): void {
        global $DB;
        ...
        $sql = "SELECT i.id, i.status
                  FROM {tool_mucatalog_item} i
                 WHERE i.type = 'program' AND i.referenceid = :programid AND i.status <> :archived
              ORDER BY i.id ASC";
        $rs = $DB->get_recordset_sql($sql, ['programid' => $programid, 'archived' => util::STATUS_ARCHIVED]);
        foreach ($rs as $item) {
    ...
    public static function get_description(stdClass $item): ?string {
        ...
        if (!$program || trim($program->description) === '') {
            return null;
        }

        $syscontext = \context_system::instance();

        $description = file_rewrite_pluginfile_urls($program->description, 'pluginfile.php', $syscontext->id, 'tool_mucatalog', 'item_description', $item->id);
        return format_text($description, $program->descriptionformat, ['context' => $syscontext]);
    }
    ...
    public static function get_reference(int $itemid): string {
        ...
        $sql = "SELECT p.id, p.fullname, p.contextid
                  ...
                  JOIN {tool_muprog_program} p ON p.id = i.referenceid
                 WHERE i.id = :id AND i.type = 'program'";
        $program = $DB->get_record_sql($sql, ['id' => $itemid]);
        if (!$program) {
    ...
    public static function get_image_url(stdClass $item): string {
        ...
        $program = $DB->get_record('tool_muprog_program', ['id' => $item->referenceid], 'id, presentationjson');
        if ($program) {
    ...
    public static function send_image(stdClass $item): never {
        ...
        if (!empty($presentation['image'])) {
            $fs = get_file_storage();
            $file = $fs->get_file($syscontext->id, 'tool_muprog', 'image', $program->id, '/', $presentation['image']);
            if ($file) {
    ...
    public static function send_description_file(stdClass $item, string $filepath, string $filename): never {
        ...
        $syscontext = \context_system::instance();
        $fs = get_file_storage();
        $file = $fs->get_file($syscontext->id, 'tool_muprog', 'description', $program->id, $filepath, $filename);
        if ($file) {
    ...
    public static function send_geopattern(stdClass $item): never {
        $geopattern = \tool_muprog\local\program::get_image_geopattern($item->referenceid);
    ...
    public static function is_user_registered(stdClass $item, int $userid): bool {
        ...
        $sql = new sql(
            "SELECT 'x'
              ...
              WHERE pa.userid = :userid AND p.id = :programid AND p.archived = 0 AND pa.archived = 0",
            ['userid' => $userid, 'programid' => $item->referenceid]
        );
    ...
    public static function get_open_url(stdClass $item): ?url {
        ...
        if (self::is_user_registered($item, $USER->id)) {
            return new url('/admin/tool/muprog/my/program.php', ['id' => $program->id]);
        }
