...
final class catalogue {
     ...
     * @return array
     */
    public static function get_sections(int $userid, bool $frontpageonly, ?int $tenantid, string $fields = 's.*'): array {
        global $DB;
    ...
    /**
     * Returns collections user can see.
     *
     * @param int $userid
     * @param bool $frontpageonly
     * @param int|null $tenantid
     * @return array
     */
    public static function get_collections(int $userid, bool $frontpageonly, ?int $tenantid): array {
        global $DB;
     ...
     * Returns items.
     *
     * @param int $sectionid positive int is section id, negative is collection id (visibility is not validated)
     * @param int $userid
     * @param int|null $tenantid
     ...
     */
    public static function fetch_items(int $sectionid, int $userid, ?int $tenantid, string $orderby, int $limitfrom, int $limitnum, array $filters = []): array {
        global $DB;
        ...
        if ($sectionid > 0) {
            $sql = $sql->replace_comment('sectionvisiblejoin', "");
            $sql = $sql->replace_comment('sectionvisiblewhere', "AND s.id = ?", [$sectionid]);
    ...
    /**
     * Can current user browse the section?
     *
            ...
            if (!$collection->uservisible) {
                $sql = new sql(
                    "SELECT 'x'
                       FROM {cohort_members} cm
                       JOIN {tool_mucatalog_collection_cohortvisible} cv ON cv.cohortid = cm.cohortid
                      WHERE cm.userid = :userid AND cv.collectionid = :colelctionid",
