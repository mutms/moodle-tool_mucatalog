...
final class section extends base {
    ...
    protected function get_default_entity_title(): lang_string {
        return new lang_string('section', 'tool_mucatalog');
    }
    ...
    public function get_context_join(): string {
        ...
        return "JOIN {context} {$contextalias} ON {$contextalias}.id = {$sectionalias}.contextid";
    }
    ...
    public function get_context_map_join(\context $context): string {
        ...
        return "JOIN {tool_mulib_context_map} {$contextmapalias} ON
                     {$contextmapalias}.contextid = {$sectionalias}.contextid AND {$contextmapalias}.relatedcontextid = {$context->id}";
    }
    ...
    protected function get_all_columns(): array {
        ...
        $columns[] = (new column(
            'name',
            new lang_string('section_name', 'tool_mucatalog'),
            $this->get_entity_name()
        ))
            ->add_joins($this->get_joins())
            ->set_type(column::TYPE_TEXT)
            ->add_fields("{$sectionalias}.name, {$sectionalias}.id, {$sectionalias}.contextid")
            ...
            ->set_callback(static function (?string $value, \stdClass $row): string {
                ...
                if (has_capability('tool/mucatalog:view', $context)) {
                    $url = new url('/admin/tool/mucatalog/management/section.php', ['id' => $row->id]);
        ...
        $columns[] = (new column(
            'status',
            new lang_string('section_status', 'tool_mucatalog'),
            $this->get_entity_name()
        ))
            ->add_joins($this->get_joins())
            ->set_type(column::TYPE_INTEGER)
            ->add_field("{$sectionalias}.status")
            ->set_is_sortable(true)
            ->add_callback(static function (?int $value, \stdClass $row): string {
                if ($value === null) {
                    return '';
        ...
        $columns[] = (new column(
            ...
            ->add_field("{$sectionalias}.guestvisible")
            ->set_is_sortable(true)
        ...
        $columns[] = (new column(
            ...
            ->add_joins($this->get_joins())
            ->set_type(column::TYPE_BOOLEAN)
            ->add_field("{$sectionalias}.uservisible")
            ->set_is_sortable(true)
            ->set_callback([format::class, 'boolean_as_text']);
        ...
        $columns[] = (new column(
            ...
            ->add_joins($this->get_joins())
            ->set_type(column::TYPE_BOOLEAN)
            ->add_field("{$sectionalias}.hiddenfromtenants")
            ->set_is_sortable(true)
            ->set_callback([format::class, 'boolean_as_text']);

        $columns[] = (new column(
            'context',
            new lang_string('section_category', 'tool_mucatalog'),
            $this->get_entity_name()
        ))
            ->add_joins($this->get_joins())
            ->add_join($this->get_context_join())
            ->set_type(column::TYPE_INTEGER)
            ->add_field("{$sectionalias}.contextid")
        ...
        $columns[] = (new column(
            ...
            ->add_callback(static function (?int $value, \stdClass $row): string {
                ...
                $cohorts = \tool_mucatalog\local\section::get_cohortvisible_menu($row->id);
                if (!$cohorts) {
                    return '-';
                }
        ...
        $columns[] = (new column(
            ...
            ->add_joins($this->get_joins())
            ->set_type(column::TYPE_INTEGER)
            ->add_fields("{$sectionalias}.id, {$sectionalias}.contextid")
            ->set_is_sortable(false)
            ->add_callback(static function (?int $value, \stdClass $row): string {
                global $DB;
                ...
                $count = $DB->count_records('tool_mucatalog_item', ['sectionid' => $row->id]);

                $context = \context::instance_by_id($row->contextid, IGNORE_MISSING);
                if ($context && has_capability('tool/mucatalog:view', $context)) {
                    $url = new url('/admin/tool/mucatalog/management/section_items.php', ['id' => $row->id]);
