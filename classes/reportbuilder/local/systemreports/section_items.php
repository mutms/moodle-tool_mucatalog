...
/**
 * Embedded section items report.
 *
 * @package     tool_mucatalog
 * @copyright   2026 Petr Skoda
 * @license     https://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
final class section_items extends system_report {
    /** @var \stdClass */
    protected $section;
    /** @var item */
    ...
    protected function initialise(): void {
        ...
        $this->section = $DB->get_record('tool_mucatalog_section', ['id' => $this->get_parameter('sectionid', null, PARAM_INT)], '*', MUST_EXIST);
        ...
        $this->add_base_fields("{$this->itemalias}.id, {$this->itemalias}.status, {$this->sectionalias}.contextid, {$this->itemalias}.type, {$this->sectionalias}.status AS sectionstatus");

        $param = database::generate_param_name();
        $this->add_base_condition_sql(
            "{$this->sectionalias}.id = :$param",
            [$param => $this->section->id]
        );
    ...
    protected function can_view(): bool {
        ...
        $exists = $DB->record_exists(
            ...
            ['id' => $this->get_parameter('sectionid', null, PARAM_INT), 'contextid' => $context->id],
        );
        if (!$exists) {
            return false;
        }
    ...
    public function get_row_class(\stdClass $row): string {
        return ($row->status != \tool_mucatalog\local\util::STATUS_ACTIVE || $row->sectionstatus != \tool_mucatalog\local\util::STATUS_ACTIVE) ? 'text-muted' : '';
    }
    ...
    protected function add_actions(): void {
        ...
        $this->add_action($link->create_report_action()
            ->add_callback(static function (\stdclass $row): bool {
                ...
                $itemclass = \tool_mucatalog\local\item::get_type_classname($row->type);
                if (!$itemclass || !$itemclass::is_activate_possible($row)) {
                    return false;
                }
        ...
        $this->add_action($link->create_report_action()
            ->add_callback(static function (\stdclass $row): bool {
                ...
                $itemclass = \tool_mucatalog\local\item::get_type_classname($row->type);
                if (!$itemclass || !$itemclass::is_archive_possible($row)) {
                    return false;
                }
        ...
        $this->add_action($link->create_report_action()
            ->add_callback(static function (\stdclass $row): bool {
                ...
                $itemclass = \tool_mucatalog\local\item::get_type_classname($row->type);
                if (!$itemclass || !$itemclass::is_restore_possible($row)) {
                    return false;
                }
