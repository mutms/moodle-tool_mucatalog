...
$currenturl = new url('/admin/tool/mucatalog/management/item_activate.php', ['id' => $item->id]);
...
if (!$itemclass || !$itemclass::is_activate_possible($item)) {
    redirect($returnurl);
}

$form = new \tool_mucatalog\local\form\item_activate(null, ['section' => $section, 'context' => $context, 'item' => $item]);
if ($form->is_cancelled()) {
    $form->ajax_form_cancelled($returnurl);
} else if ($data = $form->get_data()) {
    $item = $itemclass::activate($data->id);
