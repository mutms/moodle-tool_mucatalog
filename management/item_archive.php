...
$currenturl = new url('/admin/tool/mucatalog/management/item_archive.php', ['id' => $item->id]);
...
if (!$itemclass || !$itemclass::is_archive_possible($item)) {
    redirect($returnurl);
}

$form = new \tool_mucatalog\local\form\item_archive(null, ['section' => $section, 'context' => $context, 'item' => $item]);
if ($form->is_cancelled()) {
    $form->ajax_form_cancelled($returnurl);
} else if ($data = $form->get_data()) {
    $item = $itemclass::archive($data->id);
