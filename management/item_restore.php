...
/**
 * Restore archived catalogue item.
 *
...
if (!$itemclass || !$itemclass::is_restore_possible($item)) {
    redirect($returnurl);
}

$form = new \tool_mucatalog\local\form\item_restore(null, ['section' => $section, 'context' => $context, 'item' => $item]);
if ($form->is_cancelled()) {
    $form->ajax_form_cancelled($returnurl);
} else if ($data = $form->get_data()) {
    $item = $itemclass::restore($data->id);
